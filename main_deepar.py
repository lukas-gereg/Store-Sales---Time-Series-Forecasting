...
from models.deepar_model import DeepAR, negative_log_likelihood_loss
...
def main():
    ...
    sequence_length = 30
    test_size = 0.1  # 5% of the data for testing
    val_size = 0.2   # 10% of the data for validation
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets in train_loader:
            ...
            # Calculate loss
            loss = negative_log_likelihood_loss(mu, sigma, targets)
            loss.backward()
            optimizer.step()
        ...
        if val_loss < best_val_loss:
            ...
            trigger_times = 0
            torch.save(model.state_dict(), 'best_deepar_model.pth')  # Save the best model
        else:
            trigger_times += 1
    ...
    # Load the best model
    model.load_state_dict(torch.load('best_deepar_model.pth'))
    model.eval()
    ...
    with torch.no_grad():
        for inputs, targets in test_loader:
            inputs, targets = inputs.to(device), targets.to(device)
            mu, sigma = model(inputs)
            predictions.extend(mu.cpu().numpy())  # Use predicted mean (mu)
